...
import { Search, ScanBarcode, Minus, Plus, Trash2, LogOut, Check, ArrowRight, ShoppingBag, Printer, UserCircle, X, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 md:h-16 border-b border-border bg-card px-3 md:px-4 flex items-center justify-between shadow-sm z-10 gap-2">
          <img src={logoImg} alt="مشاتل القادري" className="h-8 md:h-10 w-auto object-contain shrink-0" />
          <div className="relative flex-1 max-w-xl mx-2">
            <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} />
            <Input
              ref={searchInputRef}
              placeholder="ابحث أو امسح الباركود..."
              className="pl-4 pr-9 rounded-full bg-muted/50 border-none focus-visible:ring-1 text-sm"
              value={search}
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0">
            <Popover open={isCustomerPickerOpen} onOpenChange={setIsCustomerPickerOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-full gap-1.5 text-xs md:text-sm px-2 md:px-3">
                  <UserCircle size={16} />
                  <span className="hidden sm:inline">{selectedCustomer ? selectedCustomer.name : "زبون عام"}</span>
                </Button>
              </PopoverTrigger>
            ...
            {user.role === "admin" && (
              <Button variant="ghost" size="sm" className="hidden md:flex" onClick={() => setLocation("/dashboard")}>
                لوحة التحكم
                <ArrowRight className="mr-1" size={14} />
              </Button>
            )}
            <Button variant="ghost" size="icon" className="hidden md:flex text-muted-foreground hover:text-destructive" onClick={logout} title="تسجيل الخروج">
              <LogOut size={18} />
            </Button>
          </div>
        </header>
        ...
        {/* Categories */}
        <div className="px-3 md:px-4 py-2 md:py-3 bg-card border-b border-border overflow-x-auto flex gap-2 hide-scrollbar">
          <Button
            variant={selectedCategory === null ? "default" : "outline"}
            className="rounded-full whitespace-nowrap text-xs md:text-sm h-8 md:h-9"
        ...
        <ScrollArea className="flex-1 p-3 md:p-4 bg-muted/20">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4 pb-24 md:pb-4">
            {filteredProducts.map(product => (
              <div
                key={product.id}
                onClick={() => addToCart(product)}
                className="bg-card rounded-2xl p-2.5 md:p-3 border border-border/50 shadow-sm cursor-pointer hover:shadow-md hover:border-primary/30 transition-all hover:-translate-y-1 group active:scale-95"
              >
                <div className="aspect-square bg-muted rounded-xl mb-2 md:mb-3 overflow-hidden flex items-center justify-center">
                  {product.imageUrl ? (
                ...
                <h3 className="font-medium text-xs md:text-sm line-clamp-2 min-h-[32px] md:min-h-[40px] text-card-foreground leading-tight">
                  {product.nameAr}
                </h3>
                <p className="font-bold text-primary mt-1.5 flex justify-between items-center text-xs md:text-sm">
                  <span>{product.salePrice.toFixed(2)} د.أ</span>
                  <span className="text-muted-foreground bg-muted px-1.5 py-0.5 rounded-md font-normal text-[10px] md:text-xs">{product.quantity}</span>
                </p>
              </div>
            ))}
            {isLoadingProducts && (
        ...
        {/* Mobile cart FAB */}
        <div className="md:hidden fixed bottom-0 inset-x-0 px-4 pb-4 pt-2 bg-gradient-to-t from-background via-background/95 to-transparent z-40">
          <Button
            className="w-full h-14 text-base font-bold rounded-2xl shadow-xl gap-3"
            onClick={() => setMobileCartOpen(true)}
          >
            <ShoppingBag size={22} />
            <span>السلة</span>
            {cart.length > 0 && (
              <span className="bg-white/20 rounded-full px-2 py-0.5 text-sm">{cart.length} صنف</span>
            )}
            {total > 0 && (
              <span className="mr-auto font-bold">{total.toFixed(2)} د.أ</span>
            )}
          </Button>
        </div>
      </div>
      ...
      <div className="hidden md:flex w-96 shrink-0 flex-col bg-card border-r border-border shadow-xl">
      ...
      <Sheet open={mobileCartOpen} onOpenChange={setMobileCartOpen}>
        <SheetContent side="bottom" className="h-[85vh] flex flex-col p-0 rounded-t-3xl" dir="rtl">
        ...
        <DialogContent className="sm:max-w-md w-[95vw]" dir="rtl">
          ...
          <div className="py-4 md:py-6">
            <div className="text-center mb-6 md:mb-8">
              ...
              <div className="text-3xl md:text-4xl font-bold text-primary">{total.toFixed(2)} د.أ</div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  className={`p-3 md:p-4 rounded-xl border-2 flex flex-col items-center gap-1.5 md:gap-2 transition-all ${
      ...
      <Dialog open={isSuccessOpen} onOpenChange={setIsSuccessOpen}>
        <DialogContent className="sm:max-w-sm w-[95vw]" dir="rtl">
